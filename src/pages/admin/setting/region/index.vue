...
viewWrapper.setPageTitle('Region')
...
const isRegional = ref(props.regional) ?? null
...
const fetchData: VFlexTableWrapperDataResolver = async ({
  ...
  const { _data: data } = await $fetch.raw(`/api/regional`, {
    query: {
...
const onAdd = async () => {
  ...
  try {
    await $fetch.raw(`/api/regional`, { method: 'POST', body: {
      name: name.value,
    } }).then(() => {
...
const onEdit = async () => {
  ...
  isLoading.value = true
  try {
    await $fetch.raw(`/api/regional/${isRegional.value.id}`, { method: 'PUT', body: {
      name: isRegional.value.name,
    } }).then(() => {
      isLoading.value = false
      componentKey.value += 1
...
const onDelete = async () => {
  ...
  isLoading.value = true
  try {
    await $fetch.raw(`/api/regional/${isRegional.value.id}`, { method: 'DELETE' }).then(() => {
      isLoading.value = false
      componentKey.value += 1

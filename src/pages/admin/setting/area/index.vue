...
import { useRegional } from '/@src/services/get-regional'
...
const name = ref('')
const regional_id = ref()
const notyf = useNotyf()
const viewWrapper = useViewWrapper()
viewWrapper.setPageTitle('Area Group')
...
const isArea = ref(props.area) ?? null
...
const fetchData: VFlexTableWrapperDataResolver = async ({
  ...
  const { _data: data } = await $fetch.raw(`/api/area-group`, {
    query: {
...
const onAdd = async () => {
  ...
  try {
    await $fetch.raw(`/api/area-group`, { method: 'POST', body: {
      name: name.value,
...
const onEdit = async () => {
  ...
  isLoading.value = true
  try {
    await $fetch.raw(`/api/area-group/${isArea.value.id}`, { method: 'PUT', body: {
      name: isArea.value.name,
      regional_id: isArea.value.regional_id,
    } }).then(() => {
      isLoading.value = false
      componentKey.value += 1
...
const onDelete = async () => {
  ...
  isLoading.value = true
  try {
    await $fetch.raw(`/api/area-group/${isArea.value.id}`, { method: 'DELETE' }).then(() => {
      isLoading.value = false
      componentKey.value += 1
...
<template>
  ...
  <div>
        ...
        >
          ...
          <template #body-cell="{ row, column }">
            ...
            <template v-if="column.key === 'region'">
              <div>
                <span class="dark-text">{{ row?.regionals.name }}</span>
              </div>
            </template>
    ...
    >
      <template #content>
        <VField label="Pilih Region" class="is-autocomplete-select">
          <VControl icon="feather:calendar">
            <Multiselect
              ...
              placeholder="region"
            />
            <p v-if="errors?.regional_id" class="help is-danger">
              {{ errors?.regional_id }}
            </p>
          </VControl>
        </VField>

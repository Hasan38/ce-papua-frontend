...
import { type Info } from '/@src/models/info'
...
const isInfo = ref(props.info) ?? null
...
const fetchData: VFlexTableWrapperDataResolver = async ({
  ...
  const { _data: data } = await $fetch.raw(`/api/info`, {
    query: {
          ...
          <template #body-cell="{ row, column }">
            <template v-if="column.key === 'title'">
              ...
              <div>
                ...
                <span class="dark-text">{{ row?.type }}</span>
              </div>
            </template>
            <template v-if="column.key === 'area'">
              <div>
                <span class="dark-text">{{ row?.area }}</span>
                <span class="dark-text">{{ row?.type }}</span>
              </div>
            </template>
            <template v-if="column.key === 'link'">
              <span class="dark-text">{{ row?.link }}</span>
            </template>
        ...
        >
          <VControl>
            <VSelect v-model="type">
              ...
              <VOption value="jadwal-kerja-ce">
                Jadwal Kerja Ce
              </VOption>
              <VOption value="jadwal-kerja-hd">
                Jadwal Kerja HD
              </VOption>
            </VSelect>
            <p v-if="errors?.type" class="help is-danger">
              {{ errors?.roles }}
            </p>
          </VControl>
        </VField>

...
viewWrapper.setPageTitle('Edit Archiev')
useHead({
  title: computed(() => 'Edit Archiev'),
})
...
async function getVideo() {
  await $fetch.raw(`/api/tutorial/${id}`, {
    method: 'GET' }).then((res) => {
    isLoading.value = false
    machine_type.value = res._data.data.machine_type
    type.value = res._data.data.type
    customer.value = res._data.data.customer
    title.value = res._data.data.title
    content.value = res._data.data.content
    link.value = res._data.data.link
  }).catch ((e) => {
    isLoading.value = false
    console.log(e)
  })
...
const onSubmit = async () => {
  ...
  await $fetch.raw(`/api/tutorial/${id}`, {
    ...
    isLoading.value = false
    getRouter(type.value)
    notyf.success('data berhasil di ubah')
  }).catch ((e) => {
    isLoading.value = false
    if (e.status === 422) {
